...
write_files:
  ...
  - path: /home/minecraft/.aws/config
    ...
    content: |
      [default]
      region = ${AWS_REGION}
      ${S3_ENDPOINT ? `endpoint_url = ${S3_ENDPOINT}` : ''}
  ...
  - path: /opt/minecraft/startup.sh
    ...
    content: |
      ...
      if [ ! -f "/opt/minecraft/server.jar" ]; then
        echo "[startup] No server.jar found, downloading..."
        ...
        if [ "$SOFTWARE" = "forge" ]; then
          ...
          if [ "$IS_MODERN_FORGE" = "true" ]; then
            ...
            [ -n "$FORGE_JAR" ] && mv "$FORGE_JAR" server.jar && rm -f server-installer.jar
          fi
        else
      ...
      ${isFabric ? `
      if [ "$SOFTWARE" = "fabric" ] && [ ! -f "/opt/minecraft/server.jar" ]; then
  ...
  - path: /etc/systemd/system/minecraft.service
    ...
    content: |
      ...
      ExecStop=/usr/bin/mcrcon -H 127.0.0.1 -P 25575 -p "$RCON_PASSWORD" stop
      Restart=always
      User=minecraft
...
runcmd:
  ...
  - wget -O /usr/local/bin/mcrcon https://github.com/Tiiffi/mcrcon/releases/download/v0.7.2/mcrcon-0.7.2-linux-x86-64.tar.gz
  - tar -xzf /usr/local/bin/mcrcon -C /usr/local/bin/
  - chmod +x /usr/local/bin/mcrcon
  - rm /usr/local/bin/mcrcon.tar.gz
  - apt-get update
  - curl -fsSL https://deb.nodesource.com/setup_20.x | bash -
  - apt-get install -y nodejs
  - npm install -g pm2
  - cd /opt/minecraft
  - sudo -u minecraft aws s3 cp s3://${S3_BUCKET}/spawnly/scripts/mc-sync.sh /usr/local/bin/mc-sync.sh ${endpointCliOption}
  - sudo -u minecraft aws s3 cp s3://${S3_BUCKET}/spawnly/scripts/mc-sync-from-s3.sh /usr/local/bin/mc-sync-from-s3.sh ${endpointCliOption}
  - sudo -u minecraft aws s3 cp s3://${S3_BUCKET}/spawnly/scripts/status-reporter.js /opt/minecraft/status-reporter.js ${endpointCliOption}
  - sudo -u minecraft aws s3 cp s3://${S3_BUCKET}/spawnly/scripts/console-server.js /opt/minecraft/console-server.js ${endpointCliOption}
  - sudo -u minecraft aws s3 cp s3://${S3_BUCKET}/spawnly/scripts/properties-api.js /opt/minecraft/properties-api.js ${endpointCliOption}
  - sudo -u minecraft aws s3 cp s3://${S3_BUCKET}/spawnly/scripts/metrics-server.js /opt/minecraft/metrics-server.js ${endpointCliOption}
  - sudo -u minecraft aws s3 cp s3://${S3_BUCKET}/spawnly/scripts/file-api.js /opt/minecraft/file-api.js ${endpointCliOption}
  - chmod +x /usr/local/bin/mc-sync.sh
  - chmod +x /usr/local/bin/mc-sync-from-s3.sh
  - sudo -u minecraft npm install --no-audit --no-fund ws express multer archiver cors
  - certbot --nginx -d ${escapedSubdomain}.spawnly.net --non-interactive --agree-tos --email admin@spawnly.net
  - systemctl daemon-reload
  - systemctl enable minecraft
  - systemctl start minecraft
  - systemctl restart nginx
  - echo "0 0,12 * * * root certbot renew --quiet" >> /etc/crontab
  - sudo -u minecraft pm2 start /opt/minecraft/status-reporter.js --name status-reporter
  - sudo -u minecraft pm2 start /opt/minecraft/console-server.js --name console-server
  - sudo -u minecraft pm2 start /opt/minecraft/properties-api.js --name properties-api
  - sudo -u minecraft pm2 start /opt/minecraft/metrics-server.js --name metrics-server
  - sudo -u minecraft pm2 start /opt/minecraft/file-api.js --name file-api
  - sudo -u minecraft pm2 save
  - ufw allow 22
  - ufw allow 25565
  - ufw allow 25575
  - ufw allow 3002
