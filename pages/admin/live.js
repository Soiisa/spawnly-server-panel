  ...
  return (
    <div className="h-screen w-screen bg-[#050510] overflow-hidden text-cyan-50 font-mono relative selection:bg-cyan-500/30">
      ...
      <style jsx global>{`
        ...
        .cyber-clip { clip-path: polygon(0 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%); }
        .cyber-clip-inv { clip-path: polygon(20px 0, 100% 0, 100% 100%, 0 100%, 0 20px); }
        .scanlines {
          background: linear-gradient(to bottom, rgba(255,255,255,0), rgba(255,255,255,0) 50%, rgba(0,0,0,0.2) 50%, rgba(0,0,0,0.2));
          background-size: 100% 4px; animation: scroll 10s linear infinite; pointer-events: none;
        }
        ...
        .neon-orange-glow { text-shadow: 0 0 10px #f97316, 0 0 20px #f97316; }
      ...
      <header className="absolute top-0 left-0 w-full p-6 flex justify-between items-end border-b border-cyan-900/50 bg-[#050510]/80 backdrop-blur-md z-40">
        <div>
          ...
          <p className="text-xs text-cyan-600 mt-1 tracking-[0.3em]">QUANTUM INFRASTRUCTURE MONITOR // V.9.1</p>
        </div>
      ...
      <main className="h-full pt-32 pb-8 px-8 grid grid-cols-12 grid-rows-6 gap-6 relative z-10">
        ...
        >
          ...
          <div className="grid grid-cols-2 h-full gap-4">
            ...
            {/* Left: Servers */}
            <div className="flex flex-col justify-center border-r border-cyan-900/50 pr-4">
              <h3 className="text-cyan-500 text-[10px] tracking-[0.2em] font-bold mb-2">ACTIVE NODES</h3>
              <div className="flex items-baseline gap-1">
                <span className="text-6xl font-black text-white neon-text-glow leading-none">{data.servers.active}</span>
                <span className="text-lg text-cyan-700 font-bold">/ {data.servers.total}</span>
              </div>
              <div className="w-full bg-cyan-900/30 h-1 skew-x-12 mt-4">
                  <motion.div 
            ...
            <div className="flex flex-col justify-center pl-2">
              <h3 className="text-orange-500 text-[10px] tracking-[0.2em] font-bold mb-2">LIVE PLAYERS</h3>
              <span className="text-6xl font-black text-white neon-orange-glow leading-none">
                {data.players || 0}
              </span>
              <div className="text-[10px] text-orange-500/50 mt-2">CONNECTED CLIENTS</div>
            </div>
        ...
        <div className="col-span-8 row-span-3 grid grid-cols-2 gap-6">
            ...
            >
               ...
               <div className="space-y-1.5 opacity-80">
                  ...
                  <p className="text-cyan-700">[{new Date().toLocaleTimeString()}] <span className="text-white">Syncing state ({data.players || 0} players online)...</span> [OK]</p>
