...
// --- INTERNAL COMPONENT: PARTNER REFERRAL CODE FIELD ---
const PartnerCodeField = ({ value, onChange, onCheck, checking, info, t }) => (
  <div className="mb-5">
    <label className="block text-[10px] font-bold text-slate-400 mb-1.5 uppercase tracking-widest">
      {t('partner.label', { defaultValue: 'Partner code (optional)' })}
    </label>
    <div className="flex gap-2">
      <input
        ...
        onBlur={onCheck}
        placeholder={t('partner.placeholder', { defaultValue: 'e.g. SUMMER10' })}
        className="flex-1 min-w-0 px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-mono text-sm uppercase tracking-wider focus:border-indigo-500 focus:outline-none"
      />
      <button
        type="button"
        onClick={onCheck}
        disabled={checking || !value.trim()}
        className="px-4 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-sm font-bold disabled:opacity-40 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all"
      >
        {checking ? '…' : t('partner.apply', { defaultValue: 'Apply' })}
      </button>
    </div>
    {info?.valid && (
      <p className="mt-2 text-xs font-bold text-green-600 dark:text-green-400">
        {t('partner.applied', { percent: info.bonus_percent, defaultValue: `Code applied — +${info.bonus_percent}% bonus credits` })}
      </p>
    )}
    {info && !info.valid && (
      <p className="mt-2 text-xs font-medium text-red-500">
        {t('partner.invalid', { defaultValue: 'That code is not valid.' })}
      </p>
    )}
  </div>
);
...
export default function CreditsPage() {
  ...
  // Partner referral code (shared by the one-time and subscription flows)
  const [partnerCode, setPartnerCode] = useState('');
  const [partnerInfo, setPartnerInfo] = useState(null); // null = unchecked, {valid:false} = rejected
  ...
  // Mirrors the server: volume bonus (one-time only) plus the partner bonus.
  // The webhook recalculates both -- this is purely what the buyer is shown.
  const calculateTotalCredits = (euro, partner = null, isSubscription = false) => {
    const base = Math.round(euro * 100); 
    const bonusTier = isSubscription ? null : getBonusInfo(euro);
    const bonus = bonusTier ? Math.floor(base * (bonusTier.bonus_percent / 100)) : 0;
    const partnerPercent = partner?.valid ? Number(partner.bonus_percent) : 0;
    const partnerBonus = partnerPercent ? Math.floor(base * (partnerPercent / 100)) : 0;
    return { 
      total: base + bonus + partnerBonus, 
      bonus, 
      partnerBonus,
      partnerPercent,
      totalBonus: bonus + partnerBonus,
      percent: (bonusTier ? bonusTier.bonus_percent : 0) + partnerPercent
    };
  };
  ...
  const {
    total: totalGet,
    bonus: bonusGet,
    partnerBonus: partnerBonusGet,
    partnerPercent: partnerPercentGet,
    totalBonus: totalBonusGet,
    percent: activePercent,
  } = calculateTotalCredits(depositAmount, partnerInfo);
  ...
  // Validate a partner code. The server is the authority -- this only drives
  // what the user sees; the real bonus is recalculated in the webhook.
  const checkPartnerCode = async (euroAmount) => {
    const code = partnerCode.trim();
    if (!code) { setPartnerInfo(null); return; }
    ...
    try {
      ...
      const res = await fetch('/api/partners/validate', {
        ...
        body: JSON.stringify({ code, amount: euroAmount }),
      });
      setPartnerInfo(await res.json());
    } catch (e) {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/stripe/checkout_sessions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${session.access_token}` },
        body: JSON.stringify({ amount: selectedSubAmount, isSubscription: true, partner_code: partnerCode.trim() || null })
      });
              ...
              <div className="flex flex-col w-full xl:w-auto">
                  <div className="flex flex-col sm:flex-row gap-3 w-full items-center">
                    ...
                    <PartnerCodeField
                      ...
                      onCheck={() => checkPartnerCode(selectedSubAmount)}
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 w-full text-center sm:text-left flex items-center justify-center sm:justify-start gap-1.5">
                    <SparklesIcon className="w-3.5 h-3.5 text-indigo-500" />
                    {t('subscription.conversion_rate', { defaultValue: '1€ = 100 Credits. You will receive' })} <strong className="text-indigo-600 dark:text-indigo-400">{calculateTotalCredits(selectedSubAmount, partnerInfo, true).total.toLocaleString()} {t('subscription.credits_label', { defaultValue: 'Credits' })}</strong> {t('subscription.per_month', { defaultValue: '/ month' })}.
                  </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 sm:rounded-[2.5rem] shadow-2xl w-full sm:max-w-4xl h-[100dvh] sm:h-auto sm:max-h-[95vh] overflow-y-auto sm:overflow-hidden border-0 sm:border border-gray-100 dark:border-slate-800 relative animate-in fade-in zoom-in duration-300 flex flex-col md:flex-row">
            ...
            <div className="w-full md:flex-1 p-5 md:p-10 bg-white dark:bg-slate-900 md:overflow-y-auto">
                ...
                <div className="bg-indigo-50 dark:bg-indigo-900/10 rounded-3xl p-6 md:p-8 mb-8 text-center border border-indigo-100 dark:border-indigo-900/30">
                    <div className={`inline-block px-4 py-1.5 rounded-full mb-4 transition-colors ${totalBonusGet > 0 ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400' : 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400'}`}>
                        <span className="text-xs font-bold uppercase tracking-widest">
                            {totalBonusGet > 0 ? t('modal.bonus_active', { percent: activePercent }) : t('modal.current_offer', { defaultValue: 'Current Offer' })}
                        </span>
                    </div>
                    ...
                    {totalBonusGet > 0 && (
                        <div className="mt-4 text-xs font-medium text-green-600 dark:text-green-400 flex items-center justify-center gap-1">
                            ...
                            <span>
                              {t('modal.bonus_breakdown', { base: Math.round(depositAmount * 100), bonus: bonusGet, defaultValue: 'Base: {{base}} + Bonus: {{bonus}}' })}
                              {partnerBonusGet > 0 && ' + ' + partnerBonusGet + ' (' + partnerInfo.code + ')'}
                            </span>
                        </div>
                    )}
                </div>
            ...
            <div className="w-full md:w-[400px] bg-slate-50 dark:bg-slate-950/50 border-t md:border-t-0 md:border-l border-gray-100 dark:border-slate-800 p-5 md:p-8 flex flex-col md:overflow-y-auto">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-4 space-y-3 mb-6 shadow-sm">
                    ...
                    {bonusGet > 0 && <div className="flex justify-between text-sm text-green-600 dark:text-green-400"><span>{t('checkout.bonus_applied', { defaultValue: 'Bonus Applied' })}</span><span className="font-mono">+{activePercent - partnerPercentGet}%</span></div>}
                    {partnerBonusGet > 0 && <div className="flex justify-between text-sm text-green-600 dark:text-green-400"><span>{t('checkout.partner_applied', { code: partnerInfo.code, defaultValue: 'Partner code {{code}}' })}</span><span className="font-mono">+{partnerPercentGet}%</span></div>}
                    <div className="flex justify-between text-sm"><span className="text-slate-500 dark:text-slate-400">{t('checkout.credits_received', { defaultValue: 'Credits received' })}</span><span className="font-mono font-bold text-slate-900 dark:text-white">{totalGet.toLocaleString()}</span></div>
                    ...
                    {!clientSecret ? (
                         <>
                             <PartnerCodeField
                                value={partnerCode}
                                onChange={setPartnerCode}
                                onCheck={() => checkPartnerCode(depositAmount)}
                                checking={checkingCode}
                                info={partnerInfo}
                                t={t}
                             />
