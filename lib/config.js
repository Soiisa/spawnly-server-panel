...
export const getHourlyCreditCost = (ram) => {
  return Number(ram); // 1 GB = 1 credit/hour
};

// `servers.cost_per_hour` holds an hourly rate for both billing types: hourly
// servers store the straight per-GB rate, monthly servers store their flat
// monthly fee spread over a 720-hour month. Write that column through here.
export const getServerHourlyRate = (ram, billingType) => {
    return billingType === 'monthly'
        ? Number((getMonthlyCreditCost(ram) / 720).toFixed(4))
        : getHourlyCreditCost(ram);
};
